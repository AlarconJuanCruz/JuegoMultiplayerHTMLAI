...
// ─── SISTEMA DE SEMILLA ─────────────────────────────────────────
// La semilla define terreno, biomas y posición de árboles/rocas.
// Se genera una vez y se persiste en localStorage.
window.worldSeed = 0;
window.seedCode = '';
...
// Derivar parámetros de terreno y biomas de la semilla
window.applySeed = function() {
    const s = window.worldSeed;
    // Función hash simple para derivar números deterministas desde la semilla
    function sh(offset) {
        let v = ((s ^ (s >> 7)) * 0x45d9f3b + offset * 0x9e3779b9) >>> 0;
        v = ((v ^ (v >> 15)) * 0x01234567) >>> 0;
        return (v >>> 0) / 0xFFFFFFFF;
    }
    // Fases del terreno: diferentes para cada semilla
    window._tp = {
        p0: sh(1) * Math.PI * 2,   // fase colinas grandes
        p1: sh(2) * Math.PI * 2,   // fase colinas medianas
        p2: sh(3) * Math.PI * 2,   // fase bumps
        p3: sh(4) * Math.PI * 2,   // fase detalles
        p4: sh(5) * Math.PI * 2,   // fase micro
        // Amplitud varía ±30% según semilla
        a0: 38 + sh(6) * 24,
        a1: 22 + sh(7) * 16,
        a2: 10 + sh(8) * 10,

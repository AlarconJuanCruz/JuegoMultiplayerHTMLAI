...
// ── Persistencia de worldState en disco ──────────────────────────────────────
// Cada sala guarda su worldState en /data/<roomId>.json
// Esto hace que los pozos y terreno minado sobrevivan reconexiones y reinicios.
const DATA_DIR = path.join(__dirname, 'data');
if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR, { recursive: true });

function saveRoomState(roomId, worldState) {
    try {
        const file = path.join(DATA_DIR, `${roomId}.json`);
        fs.writeFileSync(file, JSON.stringify(worldState), 'utf8');
    } catch (e) {
        console.error(`[persist] Error guardando sala ${roomId}:`, e.message);
    }
}
...
function loadRoomState(roomId) {
    try {
        ...
        console.error(`[persist] Error cargando sala ${roomId}:`, e.message);
    }
    return null;
}

// Guardar el estado global cada 60 segundos como respaldo ante crash
setInterval(() => {
    for (const [id, room] of Object.entries(rooms)) {
        if (Object.keys(room.players).length > 0) saveRoomState(id, room.worldState);
    }
}, 60_000);
...
function freshWorldState(seedCode) {
    return {
        ...
        minedCells: {},   // 'col_row' → true  (celdas de terreno destruidas por pico)
...
setInterval(() => {
    for (const [id, room] of Object.entries(rooms)) {
        if (Object.keys(room.players).length === 0) {
            // Guardar estado en disco ANTES de eliminar la sala, para que el siguiente
            // jugador que entre recupere el terreno minado, bloques colocados, etc.
            saveRoomState(id, room.worldState);
            delete rooms[id];
...
io.on('connection', (socket) => {
    ...
    socket.on('joinGame', (playerData) => {
        ...
        if (!globalRoom) {
            const id = makeRoomId();
            // Intentar restaurar el worldState guardado en disco para la sala global.
            // Esto preserva los pozos, bloques y terreno minado entre sesiones.
            ...
            globalRoom = rooms[id];
            if (savedGlobal) console.log(`[persist] Sala global restaurada desde disco (${Object.keys(savedGlobal.minedCells || {}).length} celdas minadas)`);
        }
    ...
    socket.on('worldUpdate', (data) => {
        ...
        switch (data.action) {
            ...
            case 'mine_cell': {
                // Persistir celda minada en worldState para que los jugadores
                // que entren (o mueran y reaparezcan) reciban el terreno ya excavado.
    ...
    socket.on('disconnect', () => {
        ...
        // Si la sala queda vacía, guardar estado en disco inmediatamente
        if (Object.keys(room.players).length === 0) {
            const persistKey = room.name === '__global__' ? '__global__' : room.id;
            saveRoomState(persistKey, room.worldState);
            console.log(`[persist] Sala ${room.id} guardada al quedar vacía`);
        }

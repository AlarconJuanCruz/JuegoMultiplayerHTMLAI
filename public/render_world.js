...
// blocks, trees, rocks, entities, sprites, getGroundY
// drawCharacter y roundRect están embebidos aquí directamente para evitar
// problemas de orden de carga / caché de Render.com con render_character.js.
...
window.drawCharacter = function(charData, isLocal) {
    ...
    if (charData.isDead) {
        ...
        window.ctx.translate(pCX, pCY); window.ctx.scale(0.78, 0.78);
        if (!charData.facingRight) window.ctx.scale(-1, 1);
        const maxFrames = 40; const t = Math.max(0, Math.min(1, 1 - daf / maxFrames));
        window.ctx.translate(0, t * 20); window.ctx.rotate(t * (-Math.PI / 2));
        ...
        window.ctx.strokeStyle = '#333'; window.ctx.lineWidth = 1.5; window.ctx.beginPath();
        ...
        window.ctx.stroke(); window.ctx.globalAlpha = 1; window.ctx.restore(); return;
    }
    window.ctx.save();
    const pCX = charData.x + (charData.width || 24) / 2;
    const pCY = charData.y + (charData.height || 40);
    ...
    let isJumping = false, isRunning = false, isClimbing = false;
    if (isLocal) {
        ...
        window._charIsWalking = isMovingH && !isRunning;
        charData.renderAnimTime = charData.animTime;
    } else {
        isClimbing = charData.isClimbing || false;
        const remVx = charData.vx || 0, remVy = charData.vy || 0, remGrounded = charData.isGrounded || false;
    ...
    if (isJumping) {
        ...
        const idleTime=window.game.frameCount*0.03; torsoR=Math.sin(idleTime)*0.02; headR=Math.sin(idleTime-1)*0.03; armR=0.1+Math.sin(idleTime)*0.03; armL=-0.1-Math.sin(idleTime)*0.03; elbowR=-0.1; elbowL=-0.1; bob=Math.sin(idleTime)*1;
    }
    if (charData.isDancing) {
        const dt=(window.game.frameCount-(charData.danceStart||0))*0.07;
        legR=Math.sin(dt*2.1)*1.2; kneeR=Math.max(0,Math.sin(dt*2.1-0.8)*1.8); legL=Math.sin(dt*2.1+Math.PI)*1.2; kneeL=Math.max(0,Math.sin(dt*2.1+Math.PI-0.8)*1.8);
        armR=-Math.PI*0.5+Math.sin(dt*1.7)*0.9; elbowR=Math.cos(dt*2.3)*0.7-0.3; armL=-Math.PI*0.3+Math.sin(dt*1.7+Math.PI)*0.9; elbowL=Math.cos(dt*2.3+Math.PI)*0.7-0.3;
        torsoR=Math.sin(dt*1.4)*0.25; headR=Math.sin(dt*0.9+0.5)*0.18; bob=Math.abs(Math.sin(dt*2.1))*6;
    }
    if (isClimbing) {
        const isMovingOnLadder = isLocal ? Math.abs(charData.vy||0)>0.5 : Math.abs((charData.y||0)-(charData._climbLastY||charData.y))>0.15;
        if (!isLocal) charData._climbLastY = charData.y;
        if (isMovingOnLadder) charData._climbAnim = ((charData._climbAnim||0)+0.07)%(Math.PI*2);
        const ct=charData._climbAnim||0;
        armR=-Math.PI*0.55+Math.sin(ct)*0.4; armL=-Math.PI*0.55+Math.sin(ct+Math.PI)*0.4; elbowR=0.5+Math.sin(ct)*0.2; elbowL=0.5+Math.sin(ct+Math.PI)*0.2;
        legR=Math.sin(ct+Math.PI)*0.25; kneeR=Math.max(0,Math.sin(ct+Math.PI)*0.45); legL=Math.sin(ct)*0.25; kneeL=Math.max(0,Math.sin(ct)*0.45);
        torsoR=0; headR=0.05; bob=0;
    }
    if (!isClimbing) {
        if (charData.activeTool==='torch') { armR=-Math.PI/2.5; elbowR=-0.3; }
        else if ((charData.activeTool==='bow'||charData.activeTool==='molotov')&&charData.isAiming) {
            const aimAngle=Math.atan2(targetY-(pCY-42-bob),isFacingR?(targetX-pCX):-(targetX-pCX));
            ...
            const progress=charData.attackFrame/30; armR=-Math.PI*0.9*progress+(1-progress)*0.8; elbowR=-0.2; torsoR+=0.3*progress;
        }
    }
    const skin=charData.isHit?'#ff4444':(charData.pvpHitFlash?'#ff6600':'#f1c27d');
    const shirt=charData.isHit?'#ff4444':(charData.pvpHitFlash?'#ff6600':(isLocal?'#3498db':'#686868'));
    const shirtDark=charData.isHit?'#cc0000':(charData.pvpHitFlash?'#cc3300':(isLocal?'#2980b9':'#4a4a4a'));
    const pants='#2c3e50', pantsDark='#1a252f', shoes='#222', shoesDark='#111';
    const _skin2=charData.inBackground?'#888':skin, _shirt2=charData.inBackground?'#666':shirt, _shirtD2=charData.inBackground?'#555':shirtDark;
    const _pants2=charData.inBackground?'#444':pants, _pantsD2=charData.inBackground?'#333':pantsDark;
    window.ctx.translate(0,-bob);
    // Pierna izquierda (detrás)
    window.ctx.save(); window.ctx.translate(-3,-24); window.ctx.rotate(legL); window.ctx.fillStyle=_pantsD2; window.roundRect(window.ctx,-4.5,0,9,14,4); window.ctx.translate(0,12); window.ctx.rotate(kneeL); window.ctx.fillStyle=_pantsD2; window.roundRect(window.ctx,-3.5,0,7,14,3); window.ctx.fillStyle=shoesDark; window.roundRect(window.ctx,-4.5,12,11,5,2); window.ctx.restore();
    // Torso + brazo izquierdo (detrás) + cabeza
    window.ctx.save(); window.ctx.translate(0,-24); window.ctx.rotate(torsoR);
    window.ctx.save(); window.ctx.translate(2,-20); window.ctx.rotate(armL); window.ctx.fillStyle=_shirtD2; window.roundRect(window.ctx,-3.5,0,7,12,3); window.ctx.translate(0,10); window.ctx.rotate(elbowL); window.ctx.fillStyle=_skin2; window.roundRect(window.ctx,-2.5,0,5,10,2); window.ctx.restore();
    window.ctx.fillStyle=isClimbing?_shirtD2:_shirt2; window.roundRect(window.ctx,-9,-22,18,24,6);
    window.ctx.save(); window.ctx.translate(0,-24); window.ctx.rotate(headR);
    if (isClimbing) { window.ctx.fillStyle='#3E2723'; window.roundRect(window.ctx,-10,-18,20,18,8); window.ctx.fillStyle=_skin2; window.roundRect(window.ctx,-7,-4,14,4,2); }
    else { window.ctx.fillStyle=_skin2; window.roundRect(window.ctx,-10,-18,20,20,8); window.ctx.fillStyle='#333'; window.ctx.fillRect(4,-10,3,4); window.ctx.fillStyle='#3E2723'; window.ctx.beginPath(); window.ctx.arc(0,-14,11,Math.PI,0); window.ctx.fill(); }
    window.ctx.restore(); window.ctx.restore();
    // Pierna derecha (adelante)
    window.ctx.save(); window.ctx.translate(3,-24); window.ctx.rotate(legR); window.ctx.fillStyle=_pants2; window.roundRect(window.ctx,-4.5,0,9,14,4); window.ctx.translate(0,12); window.ctx.rotate(kneeR); window.ctx.fillStyle=_pants2; window.roundRect(window.ctx,-3.5,0,7,14,3); window.ctx.fillStyle=shoes; window.roundRect(window.ctx,-4.5,12,11,5,2); window.ctx.restore();
    // Brazo derecho (adelante) + herramienta
    window.ctx.save(); window.ctx.translate(0,-24); window.ctx.rotate(torsoR); window.ctx.translate(-3,-20); window.ctx.rotate(armR); window.ctx.fillStyle=_shirt2; window.roundRect(window.ctx,-3.5,0,7,12,3); window.ctx.translate(0,10); window.ctx.rotate(elbowR); window.ctx.fillStyle=_skin2; window.roundRect(window.ctx,-2.5,0,5,10,2);
    ...
    if (!isClimbing&&charData.activeTool==='bow') {
        ...
        const pull=charData.isAiming?((charData.chargeLevel||0)/100)*15:0; window.ctx.strokeStyle='#eee'; window.ctx.lineWidth=1; window.ctx.beginPath(); window.ctx.moveTo(4.6,-14.2); window.ctx.lineTo(4.6-pull,0); window.ctx.lineTo(4.6,14.2); window.ctx.stroke();
        ...
        const C2=window.ctx, hasMolotov=isLocal?(charData.inventory.molotov||0)>0:true;
        if (hasMolotov) {
            const charge=(charData.chargeLevel||0)/100;
            if (charData.isCharging) C2.rotate(-0.6-charge*0.9); else if (charData.isAiming) C2.rotate(-0.5); else C2.rotate(Math.PI/2);
            C2.fillStyle='#2d6b1e'; C2.beginPath(); C2.roundRect(-5,-16,10,18,[3,3,5,5]); C2.fill();
            C2.fillStyle='rgba(120,220,80,0.35)'; C2.fillRect(-3,-15,3,8);
            C2.fillStyle='#1e4a12'; C2.beginPath(); C2.roundRect(-2.5,-23,5,8,2); C2.fill();
            const fs=3.5+Math.sin((window.game?.frameCount||0)*0.28+(charData.phase||0))*1.2;
            C2.fillStyle='#ff7700'; C2.beginPath(); C2.ellipse(0,-25,fs,fs*1.4,0,0,Math.PI*2); C2.fill();
            C2.fillStyle='#ffee44'; C2.beginPath(); C2.ellipse(0,-26,fs*0.55,fs*0.85,0,0,Math.PI*2); C2.fill();
        }
    } else if (!isClimbing&&(charData.activeTool==='torch'||charData.activeTool==='torch_item')) {
        const _lit=charData.torchLit, _fc=window.game?.frameCount||0;
        ...
        window.ctx.fillStyle=charData.inBackground?'#5a3010':'#a0522d'; window.ctx.fillRect(-3,-22,6,5);
        if (_lit) {
            window.ctx.fillStyle=charData.inBackground?'#888':'#e67e22'; window.ctx.beginPath(); window.ctx.arc(0,-22,5*_tf,0,Math.PI*2); window.ctx.fill();
            window.ctx.fillStyle=charData.inBackground?'#aaa':'#f1c40f'; window.ctx.beginPath(); window.ctx.arc(0,-22,3*_tf,0,Math.PI*2); window.ctx.fill();
        }
        ...
        if (charData.activeTool==='axe'||charData.activeTool==='hammer'||charData.activeTool==='pickaxe') {
            window.ctx.fillStyle=charData.inBackground?'#4a250a':'#8B4513'; window.ctx.fillRect(-2,-20,4,28);
            ...
            else{window.ctx.fillStyle='#666';window.ctx.fillRect(-6,-22,12,10);}
...
window.draw = function() {
    ...
    if (window._prevOnSurface !== _onSurface) {
        ...
        window._slDirty = true; // también fuerza rebuild de luces estáticas
    ...
    if (window.lightCtx && !_skipLight) {
        ...
        // _wts: world→screen usando la cámara actual
        function _wts(wx, wy) {
            return [(wx - _iCamX - _lW/2)*_lz + _lW/2,
                    (wy - _iCamY - _lH/2)*_lz + _lH/2];
        }

        // ── Oscuridad ambiental ────────────────────────────────────────────
        let targetDarkness = darkness * 0.65;
        if (window.getTerrainCol && window.player) {
        ...
        if (ambientDarkness < 0.05) {
            // pleno día — no hacer nada, canvas anterior es casi transparente
        } else {
            ...
            if (_slNeedRebuild) {
                ...
                if (!window._staticLightCanvas) {
                    ...
                    window._staticLightCanvas.width = _lW;
                    window._staticLightCanvas.height = _lH;
                ...
                // Bloques con luz (antorchas, hogueras)
                for (const b of window.blocks) {
                    const [bsx, bsy] = _wts(b.x + _bs*0.5, b.y + _bs*0.5);
                    if (bsx < -300 || bsx > _lW+300 || bsy < -300 || bsy > _lH+300) continue;
                    if (b.type === 'campfire' && b.isBurning) {
                        const glow = 260*_lz;
                        const cg = _slc.createRadialGradient(bsx,bsy,0,bsx,bsy,glow);
                        cg.addColorStop(0,'rgba(255,200,100,0.9)'); cg.addColorStop(0.4,'rgba(255,160,50,0.6)'); cg.addColorStop(1,'rgba(255,130,0,0)');
                        _slc.fillStyle=cg; _slc.beginPath(); _slc.arc(bsx,bsy,glow,0,Math.PI*2); _slc.fill();
                    } else if (b.type === 'placed_torch') {
                        const tg2 = 165*_lz;
                        const tg = _slc.createRadialGradient(bsx,bsy,0,bsx,bsy,tg2);
                        tg.addColorStop(0,'rgba(255,200,80,0.85)'); tg.addColorStop(0.4,'rgba(255,150,20,0.55)'); tg.addColorStop(1,'rgba(255,100,0,0)');
                        _slc.fillStyle=tg; _slc.beginPath(); _slc.arc(bsx,bsy,tg2,0,Math.PI*2); _slc.fill();
                    }
                }

                // Entidades luminosas en cueva (gólems, slimes, brood mother)
                if (!_onSurface) {
                    ...
                    for (const _e2 of (window.entities||[])) {
                        ...
                        if (_ex < -300 || _ex > _lW+300) continue;
                        if (_e2.type==='slime') {
                            const sc2=_sCol[(_e2.slimeColor||0)%3];
                            const sg2=_slc.createRadialGradient(_ex,_ey,0,_ex,_ey,35*_lz);
                            sg2.addColorStop(0,`rgba(${sc2[0]},${sc2[1]},${sc2[2]},0.45)`); sg2.addColorStop(1,`rgba(${sc2[0]},${sc2[1]},${sc2[2]},0)`);
                            _slc.fillStyle=sg2; _slc.beginPath(); _slc.arc(_ex,_ey,35*_lz,0,Math.PI*2); _slc.fill();
                        } else if (_e2.type==='golem') {
                            const [gx2,gy2]=_wts(_e2.x+_e2.width/2,_e2.y+_e2.height*0.25);
                            const gR2=90*_lz;
                            ...
                            const [bm2x,bm2y]=_wts(_e2.x+_e2.width/2,_e2.y+_e2.height*0.5);
                            const bg2=_slc.createRadialGradient(bm2x,bm2y,0,bm2x,bm2y,70*_lz);
                            bg2.addColorStop(0,'rgba(255,30,0,0.25)'); bg2.addColorStop(1,'rgba(255,0,0,0)');
                            _slc.fillStyle=bg2; _slc.beginPath(); _slc.arc(bm2x,bm2y,70*_lz,0,Math.PI*2); _slc.fill();
                        }
                    }
            ...
            // Compensar desplazamiento de cámara desde el último rebuild del canvas estático
            const _driftX = (_iCamX - (window._slCamX||_iCamX)) * _lz;
            const _driftY = (_iCamY - (window._slCamY||_iCamY)) * _lz;
            window.lightCtx.drawImage(window._staticLightCanvas, -_driftX, -_driftY);

            // ── 2. Glow del jugador (se recalcula cada rebuild del lightCanvas) ─
            if (!window.player.isDead) {
                ...
                if (hasTorch) {
                    const pGlowSize = 260 * _tf * _lz;
                    const pGrad = window.lightCtx.createRadialGradient(px,py,0,px,py,pGlowSize);
                    ...
                    const bGrad = window.lightCtx.createRadialGradient(px,py,0,px,py,bodyGlowR);
                    bGrad.addColorStop(0,'rgba(200,220,255,0.85)'); bGrad.addColorStop(0.35,'rgba(170,200,240,0.55)');
                    bGrad.addColorStop(0.65,'rgba(140,170,220,0.20)'); bGrad.addColorStop(1,'rgba(120,150,200,0)');
            ...
            if (window.game.isMultiplayer) {
                Object.values(window.otherPlayers).forEach(p => {
                    if (p.id === window.socket?.id || p.isDead) return;
                    if ((p.activeTool === 'torch' || p.activeTool === 'torch_item') && p.torchLit) {
                        const _fc2 = window.game.frameCount || 0;
                        const gs2 = 260 * _lz;
                        const [cx,cy] = _wts(p.x+(p.width||20)/2, p.y+(p.height||56)/2);
                        const pg2 = window.lightCtx.createRadialGradient(cx,cy,0,cx,cy,gs2);
                        pg2.addColorStop(0,'rgba(255,180,50,0.9)'); pg2.addColorStop(0.4,'rgba(255,150,30,0.6)'); pg2.addColorStop(1,'rgba(255,120,0,0)');
                        window.lightCtx.fillStyle=pg2; window.lightCtx.beginPath(); window.lightCtx.arc(cx,cy,gs2,0,Math.PI*2); window.lightCtx.fill();
                    }
                });
            }

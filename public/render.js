...
window.draw = function() {
    ...
    window.rocks.forEach(r => {
        if (r.x + r.width > window.camera.x && r.x < window.camera.x + window._canvasLogicW) {
            ...
            if (r.hp < r.maxHp && (Date.now() - (r.lastHitTime || 0) < 3000)) {
                window.ctx.fillStyle = 'rgba(0,0,0,0.6)'; window.ctx.fillRect(r.x, r.y - 12, r.width, 6);
                window.ctx.fillStyle = '#4CAF50'; window.ctx.fillRect(r.x + 1, r.y - 11, (r.hp / r.maxHp) * (r.width - 2), 4);
            }
            window.ctx.restore();
        }
    });
    ...
    // NUEVO TEXTO DE DAÑO: Con borde grueso y estilo súper legible
    window.damageTexts.forEach(dt => { 
        window.ctx.globalAlpha = Math.max(0, Math.min(1, dt.life)); 
        window.ctx.textAlign = 'center';
        window.ctx.font = '900 18px Inter, sans-serif'; 
        
        // Dibuja el borde (stroke) del color asignado
        window.ctx.lineWidth = 3;
        window.ctx.strokeStyle = dt.color;
        window.ctx.strokeText(dt.text, dt.x, dt.y);
        
        // Dibuja el texto interior en blanco
    ...
    const drawNameAndChat = (charData, isLocal) => {
        ...
        let currentY = pCY - 70 - bob;

        if (!isLocal) {
            window.ctx.fillStyle = 'rgba(255,255,255,0.9)'; window.ctx.font = 'bold 11px Inter, sans-serif'; window.ctx.textAlign = 'center'; 
            window.ctx.fillText(`${charData.name} (Nv. ${charData.level || 1})`, pCX, currentY);
        }

        if (charData.chatExpires && Date.now() < charData.chatExpires && charData.chatText) {
            window.ctx.font = 'bold 12px Inter, sans-serif';
            let words = charData.chatText.split(' '), lines = [], currentLine = words[0];
            for (let i = 1; i < words.length; i++) {
                if (window.ctx.measureText(currentLine + " " + words[i]).width < 160) currentLine += " " + words[i];
                else { lines.push(currentLine); currentLine = words[i]; }
            } lines.push(currentLine);

            let bH = lines.length * 16 + 10;
            let maxW = 0; lines.forEach(l => { maxW = Math.max(maxW, window.ctx.measureText(l).width); });
            let bW = maxW + 16;
            let bY = currentY - 10 - bH;

            window.ctx.fillStyle = 'rgba(0, 0, 0, 0.8)'; 
            if(window.ctx.roundRect) { window.ctx.beginPath(); window.ctx.roundRect(pCX - bW/2, bY, bW, bH, 6); window.ctx.fill(); } 
            else { window.ctx.fillRect(pCX - bW/2, bY, bW, bH); }
            window.ctx.beginPath(); window.ctx.moveTo(pCX - 5, bY + bH); window.ctx.lineTo(pCX + 5, bY + bH); window.ctx.lineTo(pCX, bY + bH + 5); window.ctx.fill();
            
            window.ctx.fillStyle = '#fff'; window.ctx.textAlign = 'center'; 
            lines.forEach((line, i) => { window.ctx.fillText(line, pCX, bY + 15 + (i * 16)); });
            
            currentY = bY; 
        }

        if (charData.isTyping) {
            let tTime = Math.floor(Date.now() / 300) % 4;
            let dots = ".".repeat(tTime === 0 ? 1 : tTime);
            let bW = 34, bH = 22;
            let bY = currentY - 10 - bH;
            
            window.ctx.fillStyle = 'rgba(0, 0, 0, 0.7)'; 
            if(window.ctx.roundRect) { window.ctx.beginPath(); window.ctx.roundRect(pCX - bW/2, bY, bW, bH, 10); window.ctx.fill(); } 
            else { window.ctx.fillRect(pCX - bW/2, bY, bW, bH); }
            
            window.ctx.fillStyle = '#fff'; window.ctx.font = 'bold 16px Inter, sans-serif'; window.ctx.textAlign = 'left'; 
            window.ctx.fillText(dots, pCX - 10, bY + 16);
        }
    };

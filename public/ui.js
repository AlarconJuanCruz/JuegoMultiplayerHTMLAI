...
(function() {
    function updateSeedDisplay() {
        ...
        if (!el) return;
        el.textContent = window.seedCode || '-----';
    }

    window.copySeedCode = function() {
        const code = document.getElementById('seed-display')?.textContent || '';
        if (code && code !== '-----') {
            navigator.clipboard.writeText(code).catch(() => {});
            const s = document.getElementById('seed-status');
            if (s) { s.textContent = '✅ Código copiado'; s.style.color = '#3ddc84'; setTimeout(() => s.textContent = '', 2500); }
        }
    };

    window.loadSeedCode = function() {
        const input = document.getElementById('seed-input');
        const status = document.getElementById('seed-status');
        if (!input?.value.trim()) return;
        const code = input.value.trim().toUpperCase();
        if (window.setSeedFromCode && window.setSeedFromCode(code)) {
            localStorage.setItem('worldSeedCode', window.seedCode);
            updateSeedDisplay();
            input.value = '';
            if (status) { status.textContent = `✅ Semilla "${window.seedCode}" aplicada`; status.style.color = '#3ddc84'; setTimeout(() => status.textContent = '', 3000); }
        } else {
            if (status) { status.textContent = '❌ Código inválido'; status.style.color = '#e74c3c'; setTimeout(() => status.textContent = '', 2000); }
        }
    };

    window.resetSeed = async function(specificCode) {
        const status = document.getElementById('seed-status');
        const setStatus = (msg, color, ms=3000) => {
            if (!status) return;
        ...
        if (window.game && window.game.isMultiplayer && window.socket) {
            ...
            try {
                ...
                localStorage.setItem('worldSeedCode', window.seedCode);
                updateSeedDisplay();
                setStatus(`🌍 Semilla ${json.seed} aplicada — recarga para jugar`, '#3ddc84', 5000);
            } catch(e) {
                setStatus('❌ No se pudo contactar el servidor', '#e74c3c');
            }
            return;
        }
...
window.switchCraftTab = function(tabName) {
    ['herramientas', 'armas', 'muebles', 'trampas'].forEach(t => {
...
window.toggleMenu = function(mName) {
    ['inventory', 'crafting', 'box', 'campfire', 'turret'].forEach(m => { 
        ...
        if(m === mName) { 
            ...
            if (m==='turret') window.renderTurretUI();
        } else {
...
window.bindCraft('btn-craft-turret', () => window.craftItem(60, 20, 0, 0, null, 'turret_item', 1));

// ─── Torreta UI ───────────────────────────────────────────────────────────────
window.currentTurret = null;

window.renderTurretUI = function() {
    if (!window.currentTurret) return;
    const el = window.getEl('turret-arrow-count');
    if (el) el.textContent = window.currentTurret.arrows || 0;
};

window.turretAction = function(action) {
    const t = window.currentTurret;
    if (!t || !window.player) return;
    const inv = window.player.inventory;
    const MAX_ARROWS = 50;
    const avail = Math.max(0, parseInt(inv.arrows) || 0);
    const space  = Math.max(0, MAX_ARROWS - Math.max(0, parseInt(t.arrows) || 0));
    let changed = 0;
    if (action === 'add1') {
        if (avail >= 1 && space >= 1) { inv.arrows = avail - 1; t.arrows = (parseInt(t.arrows) || 0) + 1; changed = 1; }
    } else if (action === 'add5') {
        const n = Math.min(5, avail, space);
        if (n > 0) { inv.arrows = avail - n; t.arrows = (parseInt(t.arrows) || 0) + n; changed = n; }
    } else if (action === 'add10') {
        const n = Math.min(10, avail, space);
        if (n > 0) { inv.arrows = avail - n; t.arrows = (parseInt(t.arrows) || 0) + n; changed = n; }
    ...
    if (changed > 0) {
        window.sendWorldUpdate('update_turret', { x: t.x, y: t.y, arrows: t.arrows });
        window.renderTurretUI();
        if (window.updateUI) window.updateUI();
        if (window.renderToolbar) window.renderToolbar();
    }
};

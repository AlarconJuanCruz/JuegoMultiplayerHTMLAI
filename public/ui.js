...
window.getEl("btn-join")?.addEventListener("click", () => {
    const ip = window.getEl("server-ip").value;
    if (ip) window.startGame(true, ip);
    else alert("Ingresa una IP válida");
});
window.getEl("btn-help")?.addEventListener("click", () => {
    const t = window.getEl("game-tips");
...
window.switchCraftTab = function (tabName) {
    ["herramientas", "armas", "muebles"].forEach((t) => {
        let el = window.getEl(`tab-${t}`);
        let btn = window.getEl(`btn-tab-${t}`);
...
window.loadServers = function () {
    ...
    if (list.length === 0) container.innerHTML = '<span style="color:#aaa;">No hay servidores guardados.</span>';
        ...
        list.forEach((ip) => {
            ...
            btn.innerText = `🔌 Conectar: ${ip}`;
            btn.onclick = () => {
                window.getEl("server-ip").value = ip;
                window.startGame(true, ip);
            };
...
window.eatFood = function (hpG, hunG) {
    ...
    if (window.player.inventory[type] > 0 && window.player.hunger < window.player.maxHunger) {
        window.player.inventory[type]--;
        window.player.hunger += hunG;
        if (window.player.hunger > window.player.maxHunger) window.player.hunger = window.player.maxHunger;
        window.player.hp += hpG;
        if (window.player.hp > window.player.maxHp) window.player.hp = window.player.maxHp;
        window.spawnParticles(window.player.x + window.player.width / 2, window.player.y, "#4CAF50", 10);
        window.updateUI();
    }
};
...
window.upgradeStat = function (stat) {
    if (window.player.statPoints > 0) {
        window.player.stats[stat]++;
        window.player.statPoints--;
...
window.transferItem = function (type, toBox, amount) {
    ...
    if (!toBox && !window.canAddItem(type, amount)) {
        window.spawnDamageText(window.player.x + window.player.width / 2, window.player.y - 20, "Inv. Lleno", "#fff");
        return;
    }
    if (toBox && window.player.inventory[type] >= amount) {
        window.player.inventory[type] -= amount;
        window.currentOpenBox.inventory[type] = (window.currentOpenBox.inventory[type] || 0) + amount;
    } else if (!toBox && window.currentOpenBox.inventory[type] >= amount) {
        window.currentOpenBox.inventory[type] -= amount;
        window.player.inventory[type] = (window.player.inventory[type] || 0) + amount;
    } else if (toBox && window.player.inventory[type] > 0) {
        let r = window.player.inventory[type];
        window.player.inventory[type] = 0;
        window.currentOpenBox.inventory[type] = (window.currentOpenBox.inventory[type] || 0) + r;
    } else if (!toBox && window.currentOpenBox.inventory[type] > 0) {
        let r = window.currentOpenBox.inventory[type];
        window.currentOpenBox.inventory[type] = 0;
        window.player.inventory[type] = (window.player.inventory[type] || 0) + r;
    }
    if (window.sendWorldUpdate)
        window.sendWorldUpdate("update_box", {
            x: window.currentOpenBox.x,
            y: window.currentOpenBox.y,
            inventory: window.currentOpenBox.inventory
        });
    window.renderBoxUI();
    window.updateUI();
};
...
window.cfAction = function (action) {
    ...
    if (action === "takeCooked" && window.currentCampfire.cooked > 0) {
        if (!window.canAddItem("cooked_meat", window.currentCampfire.cooked)) {
    ...
    if (action === "addWood" && window.player.inventory.wood > 0) {
        window.player.inventory.wood--;
        window.currentCampfire.wood++;
    }
    if (action === "addMeat" && window.player.inventory.meat > 0) {
        window.player.inventory.meat--;
        window.currentCampfire.meat++;
    }
    if (action === "takeCooked" && window.currentCampfire.cooked > 0) {
        window.player.inventory.cooked_meat =
            (window.player.inventory.cooked_meat || 0) + window.currentCampfire.cooked;
        window.currentCampfire.cooked = 0;
    }
    if (action === "toggleFire") {
        if (window.currentCampfire.isBurning) window.currentCampfire.isBurning = false;
        else if (window.currentCampfire.wood > 0) {
            window.currentCampfire.isBurning = true;
            window.currentCampfire.wood--;
            window.currentCampfire.burnTime = 1800;
        }
    }
    if (window.sendWorldUpdate)
        window.sendWorldUpdate("update_campfire", {
            x: window.currentCampfire.x,
            y: window.currentCampfire.y,
            wood: window.currentCampfire.wood,
            meat: window.currentCampfire.meat,
            cooked: window.currentCampfire.cooked,
            isBurning: window.currentCampfire.isBurning
        });
    window.renderCampfireUI();
    window.updateUI();
};
    ...
    const renderGrid = (inv, domId, toBox) => {
        ...
        for (const [type, amt] of Object.entries(inv)) {
            if (amt > 0) {
                ...
                s.onclick = () => window.transferItem(type, toBox, 1);
                s.oncontextmenu = (e) => {
                    e.preventDefault();
                    window.transferItem(type, toBox, 10);
                };
                s.innerHTML = `<div class="inv-icon" style="background-color: ${window.itemDefs[type] ? window.itemDefs[type].color : "#fff"}"></div><div class="inv-amount">${amt}</div>`;
                grid.appendChild(s);
            }
        }
    };
...
window.renderCampfireUI = function () {
    ...
    window.getEl("cf-wood-count").innerText = window.currentCampfire.wood;
    window.getEl("cf-meat-count").innerText = window.currentCampfire.meat;
    window.getEl("cf-cooked-count").innerText = window.currentCampfire.cooked;
    let btn = window.getEl("btn-cf-ignite");
    if (window.currentCampfire.isBurning) {
...
window.handleToolbarDrop = function (e, slotIndex) {
    ...
    if (
        type &&
        (window.toolDefs[type] || ["boxes", "campfire_item", "bed_item"].includes(type) || window.itemDefs[type])
    ) {
    ...
    for (let i = 0; i < 6; i++) {
        ...
        if (itemId) {
            ...
            if (window.toolDefs[itemId]) {
                let tool = window.toolDefs[itemId];
                let durHTML = "";
                if (itemId !== "hand" && window.toolMaxDurability[itemId] !== undefined) {
                    const maxDur = window.toolMaxDurability[itemId];
                    if (
                        typeof window.player.toolHealth[itemId] !== "number" ||
                        isNaN(window.player.toolHealth[itemId])
                    ) {
...
window.getReqText = function (rW, rS, rWeb, rInt) {
    let t = "";
    if (window.player.stats.int < rInt) t += `<span style="color:#ff6b6b; margin-left:5px;">Req. INT ${rInt}</span> `;
    let mW = rW - window.player.inventory.wood;
    let mWeb = rWeb - (window.player.inventory.web || 0);
    let mS = rS - (window.player.inventory.stone || 0);
    if (mW > 0) t += `<span style="color:#ff6b6b; margin-left:5px;">Falta ${mW} Mad.</span>`;
    if (mWeb > 0) t += `<span style="color:#ff6b6b; margin-left:5px;">Falta ${mWeb} Tela</span>`;
    if (mS > 0) t += `<span style="color:#ff6b6b; margin-left:5px;">Falta ${mS} Pie.</span>`;
    return t === "" ? '<span class="req-text-ok">Recursos OK</span>' : t;
};
...
window.updateUI = function () {
    ...
    if (grid) {
        ...
        itemsToRender.forEach((slotData) => {
            ...
            if (slotData.isTool) {
                slot.onclick = () => {
                    window.autoEquip(slotData.type);
                    window.toggleMenu("inventory");
                };
    ...
    elTxt("stat-points", window.player.statPoints);
    elTxt("stat-str", window.player.stats.str);
    elTxt("stat-agi", window.player.stats.agi);
    elTxt("stat-vit", window.player.stats.vit);
    elTxt("stat-sta", window.player.stats.sta);
    elTxt("stat-int", window.player.stats.int);

    document.querySelectorAll(".stat-btn").forEach((btn) => {
        btn.disabled = window.player.statPoints <= 0;
    });
    const sNotif = window.getEl("stat-notif");
    if (sNotif) sNotif.style.display = window.player.statPoints > 0 ? "flex" : "none";

    elW("hp-fill", `${(window.player.hp / window.player.maxHp) * 100}%`);
    elTxt("hp-text", `${Math.floor(window.player.hp)} / ${window.player.maxHp}`);
    elW("hunger-fill", `${(window.player.hunger / window.player.maxHunger) * 100}%`);
    elTxt("hunger-text", `${Math.floor(window.player.hunger)} / ${window.player.maxHunger}`);
    elTxt("level-text", window.player.level);
    elW("xp-fill", `${(window.player.xp / window.player.maxXp) * 100}%`);
    elTxt("xp-text", `${Math.floor(window.player.xp)} / ${window.player.maxXp}`);

    const checkBtn = (idReq, idBtn, w, s, web, intR, t) => {
        let reqDOM = window.getEl(idReq),
            btnDOM = window.getEl(idBtn);
        ...
        if (reqDOM)
            reqDOM.innerHTML = hasIt
                ? '<span style="color:#4CAF50">Fabricado</span>'
                : window.getReqText(w, s, web, intR);
        if (btnDOM)
            btnDOM.disabled =
                window.player.inventory.wood < w ||
                (window.player.inventory.stone || 0) < s ||
                (window.player.inventory.web || 0) < web ||
                window.player.stats.int < intR ||
                hasIt;
    };

    checkBtn("req-torch", "btn-craft-torch", 5, 0, 2, 0, "torch");
    checkBtn("req-axe", "btn-craft-axe", 10, 0, 0, 0, "axe");
    checkBtn("req-pickaxe", "btn-craft-pickaxe", 20, 0, 0, 0, "pickaxe");
    checkBtn("req-hammer", "btn-craft-hammer", 15, 0, 0, 0, "hammer");
    checkBtn("req-bow", "btn-craft-bow", 100, 0, 2, 0, "bow");
    checkBtn("req-sword", "btn-craft-sword", 30, 30, 0, 3, "sword");
    checkBtn("req-box", "btn-craft-box", 40, 0, 0, 1, null);
    checkBtn("req-campfire", "btn-craft-campfire", 20, 5, 0, 0, null);
    checkBtn("req-bed", "btn-craft-bed", 30, 0, 10, 0, null);

    ["btn-craft-arrow", "btn-craft-arrow2", "btn-craft-arrow5", "btn-craft-arrow10"].forEach((id, i) => {
        let c = [5, 10, 25, 50][i];
        let b = window.getEl(id);
        if (b) b.disabled = window.player.inventory.wood < c;
    });
};
...
window.updateEntityHUD = function () {
    ...
    window.entities.forEach((ent) => {
        if (
            ent.hp < ent.maxHp &&
            Date.now() - (ent.lastHitTime || 0) < 3000 &&
            ent.x + ent.width > window.camera.x &&
            ent.x < window.camera.x + window.canvas.width
        ) {
            let hpPercent = (ent.hp / ent.maxHp) * 100;
            let color =
                ent.type === "spider"
                    ? "#ff4444"
                    : ent.type === "zombie"
...
window.bindCraft = function (id, fn) {
    const el = window.getEl(id);
    if (el) el.addEventListener("click", fn);
};
window.craftItem = function (reqW, reqS, reqWeb, reqInt, tool, item, amt = 1) {
    if (
        window.player.inventory.wood >= reqW &&
        (window.player.inventory.stone || 0) >= reqS &&
        (window.player.inventory.web || 0) >= reqWeb &&
        window.player.stats.int >= reqInt
    ) {
        if (tool && !window.player.toolbar.includes(tool)) {
            window.player.inventory.wood -= reqW;
            window.player.inventory.stone -= reqS;
            window.player.inventory.web -= reqWeb;
            ...
            if (["boxes", "campfire_item", "bed_item"].includes(item) && typeof window.autoEquip === "function")
                window.autoEquip(item);
        }
        window.updateUI();
        if (typeof window.renderToolbar === "function") window.renderToolbar();
    }
};

...
// Hostear usa la IP del sitio actual (si estás en Render, usa Render. Si estás local, usa localhost)
window.getEl('btn-host').addEventListener('click', () => window.startGame(true));
...
window.refreshServerList = function() {
    ...
    if (list.length === 0) {
        container.innerHTML = '<span style="color:#aaa;">No hay servidores guardados.</span>';
    } else { 
        container.innerHTML = ''; 
        list.forEach(ip => { 
...
// --- ACCIONES DE JUGADOR (Comer, Stats) ---
...
window.invItemClick = function(type) {
    ...
    else if (type === 'boxes' || type === 'campfire_item' || type === 'bed_item') { 
        window.player.placementMode = type; 
...
// --- INTERACCIÓN CON CAJAS Y FOGATAS ---
window.transferItem = function(type, toBox, amount) {
    ...
    if (window.sendWorldUpdate) window.sendWorldUpdate('update_box', { 
        x: window.currentOpenBox.x, y: window.currentOpenBox.y, inventory: window.currentOpenBox.inventory 
    });
    window.renderBoxUI(); 
    window.updateUI();
};
...
window.cfAction = function(action) {
    ...
    if (action === 'takeCooked' && window.currentCampfire.cooked > 0) { 
        window.player.inventory.cooked_meat = (window.player.inventory.cooked_meat||0) + window.currentCampfire.cooked; 
    ...
    if (window.sendWorldUpdate) window.sendWorldUpdate('update_campfire', { 
        x: window.currentCampfire.x, y: window.currentCampfire.y, 
        wood: window.currentCampfire.wood, meat: window.currentCampfire.meat, 
        cooked: window.currentCampfire.cooked, isBurning: window.currentCampfire.isBurning 
    });
    ...
    const renderGrid = (inv, domId, toBox) => {
        ...
        for (const [type, amt] of Object.entries(inv)) {
            if (amt > 0) { 
                ...
                s.oncontextmenu = (e) => { e.preventDefault(); window.transferItem(type, toBox, 10); }; 
...
window.renderCampfireUI = function() {
    ...
    if (window.currentCampfire.isBurning) { 
        btn.innerText = "APAGAR"; btn.style.background = "#555"; 
    } else { 
        btn.innerText = "🔥 ENCENDER"; btn.style.background = "#e67e22"; 
    }
};
...
window.renderToolbar = function() {
    ...
    window.player.availableTools.forEach((toolId, index) => {
        const tool = window.toolDefs[toolId]; const div = document.createElement('div'); 
        div.className = `tool-slot ${window.player.activeTool === toolId ? 'active' : ''}`; 
        let durHTML = '';
...
// --- ACTUALIZACIÓN DE ESTADOS (HP, Hambre, XP e Inventario de 10 Slots) ---
window.updateUI = function() { 
    ...
    let elTxt = (id, t) => { let e = window.getEl(id); if(e) e.innerText = t; }; 
    let elW = (id, w) => { let e = window.getEl(id); if(e) e.style.width = w; };
    ...
    ['btn-craft-arrow','btn-craft-arrow2','btn-craft-arrow5','btn-craft-arrow10'].forEach((id,i)=>{ 
        let c=[5,10,25,50][i]; let b = window.getEl(id); if(b) b.disabled = window.player.inventory.wood < c; 
    });
};
...
window.updateEntityHUD = function() {
    ...
    window.entities.forEach(ent => {
        if (ent.x + ent.width > window.camera.x && ent.x < window.camera.x + window.canvas.width) {
            ...
            let color = ent.type === 'spider' ? '#ff4444' : (ent.type === 'zombie' ? '#228b22' : (ent.type === 'archer' ? '#8e44ad' : '#ffaa00'));
...
const craft = (reqW, reqS, reqWeb, reqInt, tool, item, amt=1) => {
    if(window.player.inventory.wood >= reqW && (window.player.inventory.stone||0) >= reqS && (window.player.inventory.web||0) >= reqWeb && window.player.stats.int >= reqInt) {
        if(tool && !window.player.availableTools.includes(tool)) { 
            ...
            window.player.availableTools.push(tool); 
            window.player.toolHealth[tool] = window.toolMaxDurability[tool]; 
        } else if(item) { 
            if (!window.canAddItem(item, amt)) { 
                window.spawnDamageText(window.player.x + window.player.width/2, window.player.y - 20, "Inventario Lleno", '#fff'); 
                return; 
            }
            window.player.inventory.wood-=reqW; 
            window.player.inventory.stone-=reqS; 
...
bindCraft('btn-craft-axe', () => craft(10, 0, 0, 0, 'axe', null)); 
bindCraft('btn-craft-pickaxe', () => craft(20, 0, 0, 0, 'pickaxe', null)); 
bindCraft('btn-craft-hammer', () => craft(15, 0, 0, 0, 'hammer', null)); 
bindCraft('btn-craft-bow', () => craft(100, 0, 2, 0, 'bow', null)); 
bindCraft('btn-craft-sword', () => craft(30, 30, 0, 3, 'sword', null)); 
bindCraft('btn-craft-box', () => craft(40, 0, 0, 1, null, 'boxes')); 
bindCraft('btn-craft-campfire', () => craft(20, 5, 0, 0, null, 'campfire_item')); 
bindCraft('btn-craft-bed', () => craft(30, 0, 10, 0, null, 'bed_item'));

bindCraft('btn-craft-arrow', () => craft(5, 0, 0, 0, null, 'arrows', 1)); 
bindCraft('btn-craft-arrow2', () => craft(10, 0, 0, 0, null, 'arrows', 2)); 
bindCraft('btn-craft-arrow5', () => craft(25, 0, 0, 0, null, 'arrows', 5)); 
bindCraft('btn-craft-arrow10', () => craft(50, 0, 0, 0, null, 'arrows', 10));
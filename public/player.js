...
window.damagePlayer = function(amount) {
    ...
    if (window.player.hp <= 0) { 
        ...
        if (bedBtn) {
            if (window.player.bedPos) bedBtn.style.display = 'inline-block';
            else bedBtn.style.display = 'none';
        }
    }
    if(window.updateUI) window.updateUI();
};
...
window.respawn = function(locationType) {
    window.player.inventory = { wood: 200, stone: 0, meat: 0, cooked_meat: 0, web: 10, arrows: 0, boxes: 0, campfire_item: 0, bed_item: 0 }; 
    window.player.level = 1; window.player.xp = 0; window.player.statPoints = 0;
    window.player.stats = { str: 0, agi: 0, vit: 0, sta: 0, int: 0 };
    window.player.availableTools = ['hand']; window.player.activeTool = 'hand'; window.player.toolHealth = {};
    ...
    if (locationType === 'bed' && window.player.bedPos) {
        ...
        if (bed) {
            // Cada cama inicializa con 10 usos invisibles. Le restamos 1.
            bed.uses = (bed.uses !== undefined ? bed.uses : 10) - 1;
            
            spawnX = bed.x;
            spawnY = bed.y - window.player.height;

            if (bed.uses <= 0) {
                window.spawnDamageText(bed.x + 15, bed.y - 20, `¡CAMA ROTA!`, '#ff4444');
                window.blocks = window.blocks.filter(b => b !== bed);
                window.player.bedPos = null;
                ...
                window.spawnDamageText(bed.x + 15, bed.y - 20, `Cama: ${bed.uses}/10`, '#FFD700');
            }
        } else {
            // Si la cama no existe (fue destruida por un jugador)
            window.player.bedPos = null;
        }
    }
